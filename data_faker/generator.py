...
def _validate_configuration(conf_file):
    ...
    for column in columns:
        ...
        if _type == 'date':
            ...
            if _from > _to:
                raise ConfigurationException('Invalid date range for {0}: [{1}; {2}]'.format(_from, _to, _label),
                                             conf_file)
        else:
            _params = _get_param(column, 'params')
            if not _params:
            ...
            if not None in [_params.get('min'), _params.get('max')]:
                ...
                if _from > _to:
                    raise ConfigurationException('Invalid numeric range for {0}: [{1}; {2}]'.format(_from, _to, _label),
                                                 conf_file)
